...
const OrderForm = () => {
  ...
  useEffect(() => {
    const total = Object.entries(productQuantity).reduce((sum, [productId, qty]) => {
      const product = products.find((p) => p.id === parseInt(productId, 10));
      return sum + (qty * (product?.price || 0));
    }, 0);
    setTotalAmount(total); // Update total dynamically
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const orderData = {
        ...formData,
        totalAmount, // Include total in payload if needed
